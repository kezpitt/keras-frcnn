...
from keras_frcnn import data_generators as data_generators
...
parser.add_option("-o", "--parser", dest="parser", help="Parser to use. One of simple or pascal_voc",
                  default="simple")
parser.add_option("-n", "--num_rois", dest="num_rois", help="Number of RoIs to process at once.", default=1024)
...
# options.train_path = "balanced_data.txt"
# options.train_path = "data_semEscala.txt"
# options.network = "darknet"
trainBaseLayers = True
...
if options.network == 'vgg':
    ...
    C.network = 'resnet50'
elif options.network == 'darknet':
    from keras_frcnn import darknet as nn
    C.network = 'darknet'
else:
...
with open("val_imgs_vgg", 'wb') as config_f:
    pickle.dump(val_imgs,config_f)
...
shared_layers = nn.nn_base(img_input, trainable=trainBaseLayers)
...
try:
    ...
    print('Could not load pretrained model weights. Starting from zero')
        ...
        while True:
            try:
                ...
                if iter_num == validation_epoch_length:
                    ...
                    write_log(tbCallBack, val_names, [loss_rpn_cls,loss_rpn_regr,loss_class_cls,loss_class_regr,curr_loss,class_acc], epoch_num)
